...
<?php		
include_once('connect.php');//Koneksi ke database	
function cekKamus($kata, $conn){
	// cari di database	
...
function Del_Inflection_Suffixes($kata){ 
	...
	if(preg_match('/^([km]u|nya|[kl]ah|pun)$/i',$kata)){ // Cek Inflection Suffixes
		$__kata = preg_match_replace('([km]u|nya|[kl]ah|pun)$/i','',$kata);
		if(preg_match('([klt]ah|pun)$/i',$kata)){ // Jika berupa particles (“-lah”, “-kah”, “-tah” atau “-pun”)
			if(preg_match('([km]u|nya)$/i',$__kata)){ // Hapus Possesive Pronouns (“-ku”, “-mu”, atau “-nya”)
				$__kata__ = preg_match_replace('([km]u|nya)$/i','',$__kata);
				return $__kata__;
			}
		}
		return $__kata;	
...
function Del_Derivation_Prefix($kata, $conn){
	$kataAsal = $kata;	
	/* ------ Tentukan Tipe Awalan ------------*/
	if(preg_match('/^(di|[ks]e)\S{1,}/',$kata)){ // Jika di-,ke-,se-
		$__kata = preg_replace('/^(di|[ks]e)/','',$kata);
		if(cekKamus($__kata, $conn)){			
			return $__kata; // Jika ada balik
		}
		$__kata__ = Del_Derivation_Suffixes($__kata, $conn);
		if(cekKamus($__kata__, $conn)){
			return $__kata__;
		}
	}
